...
## Tab "Ải" - bảng kiểu "Ủy Thác" (theo mockup người dùng cung cấp) 2 sub-tab:
...
func _build_sub_tab_buttons() -> void:
	...
	conquer_button.pressed.connect(_set_sub_tab.bind(true))
	sub_tab_row.add_child(conquer_button)

	var idle_button := Button.new()
	idle_button.text = "Treo máy"
	idle_button.pressed.connect(_set_sub_tab.bind(false))
	sub_tab_row.add_child(idle_button)

func _set_sub_tab(show_conquer: bool) -> void:
	conquer_panel.visible = show_conquer
	idle_panel.visible = not show_conquer
...
## lại thẻ map cho đúng tầng cao nhất mới.
