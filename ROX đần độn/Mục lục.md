# Mục lục

HUB điều hướng toàn bộ ghi chú thiết kế/kỹ thuật của **ROXdandon** (Godot 4.3, `C:\Users\ADMIN\Documents\ROXdandon`). Bản tóm tắt gọn để dán ra ngoài Obsidian: [[Tóm tắt dự án]]. Trạng thái hiện tại/việc còn thiếu: [[Tiến độ & Việc còn dang dở]].

Dự án này **fork từ `vrisingDanDon`** (Village Builder Demo - city-builder + auto-battler) - bỏ hẳn xây dựng thành phố/kinh tế/PvP, giữ lại phần chiến đấu PvE, đổi sang đội hình cố định 4 người kiểu Ragnarok X. Xem đầy đủ lý do/những gì mang qua-bỏ ở [[Chuyển thể từ vrisingDanDon]].

## Thiết kế Game

**Đội hình & Nhân vật**
- [[Đội hình cố định (Party)]]
- [[Chỉ số lính]]
- [[Danh sách quái (Monster Roster)]]
- [[Hoạt ảnh & Asset lính]]

**Chiến đấu**
- [[Luồng chơi (Hub → Ải → Chiến đấu)]]
- [[Công thức sát thương]]
- [[Kiểu tấn công (Attack Types)]]
- [[Hệ thống Kỹ năng]]
- [[AI chiến đấu]]
- [[Camera & Hiệu ứng trận đấu]]

**Kinh tế**
- [[Vàng (GameState)]]

**Bản đồ** (mới, xem [[Tiến độ & Việc còn dang dở]] cho ngày thêm)
- [[Thành phố & Ải AFK-Farm]] - tab Thành phố (thuần cảnh, không điều khiển được) + lịch sử map AFK-farm cũ (đã thay thế)
- [[Ủy Thác, Boss & Treo máy]] - tab Ải hiện tại (bảng Ủy Thác + Treo máy thật) + tab Ải Boss thật + hệ EXP/Cấp độ

**Nhân vật & Chỉ số** (bổ sung 2026-08-03)
- Xem [[Ủy Thác, Boss & Treo máy]] mục "Hệ EXP/Cấp độ" cho công thức lên cấp/tăng chỉ số

**Âm thanh / Cốt truyện** — vẫn chưa có gì.

## Giao diện
- [[Vỏ ứng dụng Portrait (MainShell & Navigation)]] - kiến trúc app portrait 5-tab hiện tại
- [[Hub & Quy ước UI chung]] — ⚠️ lỗi thời, chỉ giữ làm lịch sử

## Kiến trúc kỹ thuật
- [[Quy ước code & Autoload]]
- [[Lỗi đã gặp & Bài học]]
- [[Chuyển thể từ vrisingDanDon]]

## Quản lý dự án
- [[Tiến độ & Việc còn dang dở]]
- [[Tóm tắt dự án]]
- [[Cách build & chạy game]]
