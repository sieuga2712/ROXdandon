...
- `StageFarmWorld.gd` — world mô phỏng chiến đấu treo máy (dùng lại `entities/troop/TroopUnit`). **Chỉ còn lo phần "map"**: camera CỐ ĐỊNH vĩnh viễn (mô hình "treadmill" 2026-08, bỏ hẳn camera bám-1-nhân-vật kiểu "Taskbar Heroes" trước đó), vị trí spawn quái CỐ ĐỊNH (mọi đợt/mọi tầng cùng 1 chỗ), tiến độ tầng, hồi sinh/vòng đời. Toàn bộ hành vi CHIẾN ĐẤU của 1 đơn vị (targeting/né nhau/skill "Đánh mạnh"/đạn bay/rớt đồ/thưởng) đã chuyển sang `core/combat/CombatResolver.gd` — dùng CHUNG với `features/combat/BattleScene.gd` (trước 2026-08, StageFarmWorld tự viết bản chiến đấu rút gọn riêng không có skill/né nhau chỉ vì lười viết chung — giờ gộp lại, Treo máy có ĐẦY ĐỦ skill/né nhau như Ải Boss). Tự phân biệt THẮNG (quái chết sạch → báo `GameState.report_floor_cleared` rồi tự `configure()` lại với tầng kế tiếp) và THUA (party chết sạch → đánh lại đúng tầng đang treo) — xem `_check_wipe()`/`_advance_floor()`.
...
`core/app`, `core/database`, `core/shared`, `core/combat` (`EncounterGenerator` — sinh quái theo đợt/tầng; `CombatResolver` — chạy AI/sát thương/skill/né nhau/thưởng cho 1 frame, xem `StageFarmWorld._process`), `entities/troop`, `entities/stage`, `features/boss` (nhúng `BossBoardScreen` làm panel con — module DUY NHẤT được phép làm vậy), `features/combat` (mở Ủy Thác/Ải Boss qua `StageFlowController.start_stage()`).
