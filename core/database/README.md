...
get_by_id(id) -> <Data>
```
Riêng thêm: `StageDatabase.get_floors_for_map(map_id)`, `MaterialDatabase.get_by_category()/get_group_keys()/make_id()`.

## Module được phép gọi
`entities/troop`, `entities/stage`, `features/boss` (chỉ để biết kiểu `BossData`), `features/stage` (kiểu `MapData`) — chỉ để định nghĩa kiểu trả về, không gọi ngược logic của các module đó.
