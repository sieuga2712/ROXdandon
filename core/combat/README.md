...
- `EquipmentDropTable.gd` — `class_name EquipmentDropTable`, không autoload, thuần công thức xác suất (giống `EncounterGenerator`, không Node/state). Tính "rớt trang bị" khi giết quái: 50% có rớt hay không (`DROP_CHANCE`), rồi roll ĐỘC LẬP chất lượng (`QUALITY_WEIGHTS` cố định 40/30/19/10/1%) và tier (`TIER_WEIGHTS_BY_BAND`, 8 bậc theo mỗi 20 lv quái, bậc cuối 141+ giữ nguyên mãi - tier nào trọng số 0 ở bậc đó coi như chưa mở). TÁCH BIỆT khỏi rớt nguyên liệu (`CombatResolver._roll_material_drop()`). **CHƯA nối dây vào `CombatResolver`/combat thật** - chưa có resource "1 món trang bị" hay hệ kho đồ trang bị đứng sau, mới chỉ là công thức xác suất gọi ra 2 con số (quality, tier).
...
EquipmentDropTable.rolls_drop() -> bool
EquipmentDropTable.roll_quality() -> Enums.EquipmentQuality
EquipmentDropTable.roll_tier(monster_level: int) -> int  # 1..5
```

## Module được phép gọi
`EncounterGenerator`: không phụ thuộc module nào khác (chỉ `randi_range`/toán học built-in). `CombatResolver`: `core/app` (`GameState.grant_kill_exp`/`add_gold`/`add_material`), `core/database` (`MaterialDatabase`), `entities/troop` (`TroopUnit`, `Projectile`, `ImpactEffect`, `DamagePopup`). `EquipmentDropTable`: `core/config` (`Enums.EquipmentQuality`) - ngoài ra không phụ thuộc gì (thuần `randf`/toán học built-in).
